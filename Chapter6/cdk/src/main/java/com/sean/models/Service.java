...
/**
 * Creates an ECS service on top of a Network. Loads Docker images from a {@link DockerImageSource}
 * and places them into ECS task. Creates a log group for each ECS task. Creates a target group for the ECS tasks
 * that is attached to the load balancer from the Network.
 */
public class Service extends Construct {
    ...
    public Service(
            ...
            final com.sean.models.service.ServiceInputParameters serviceInputParameters,
        ...
        CfnTargetGroup targetGroup = CfnTargetGroup.Builder.create(this, "targetGroup")
                .healthCheckIntervalSeconds(serviceInputParameters.getHealthCheckIntervalSeconds())
                .healthCheckPath(serviceInputParameters.getHealthCheckPath())
                .healthCheckPort(String.valueOf(serviceInputParameters.getContainerPort()))
                .healthCheckProtocol(serviceInputParameters.getContainerProtocol())
                .healthCheckTimeoutSeconds(serviceInputParameters.getHealthCheckTimeoutSeconds())
                .healthyThresholdCount(serviceInputParameters.getHealthyThresholdCount())
                .unhealthyThresholdCount(serviceInputParameters.getUnhealthyThresholdCount())
                .targetGroupAttributes(serviceInputParameters.isStickySessionsEnabled() ? stickySessionConfiguration : Collections.emptyList())
                .targetType("ip")
                .port(serviceInputParameters.getContainerPort())
                .protocol(serviceInputParameters.getContainerProtocol())
        ...
        LogGroup logGroup = LogGroup.Builder.create(this, "ecsLogGroup")
                ...
                .retention(serviceInputParameters.getLogRetention())
        ...
        Role ecsTaskExecutionRole = Role.Builder.create(this, "ecsTaskExecutionRole")
                ...
                .inlinePolicies(
                        ...
                        .collect(Collectors.toMap(data -> (String) data[0], data -> (PolicyDocument) data[1])) )
                .build();
...
//                .build();
        ...
        if (!serviceInputParameters.getTaskRolePolicyStatements().isEmpty()) {
            roleBuilder.inlinePolicies(Stream.of(new Object[][] {
                {
                    applicationEnvironment.prefix("ecsTaskRolePolicy"),
                    PolicyDocument.Builder.create()
                            .statements(serviceInputParameters.getTaskRolePolicyStatements())
                            .build()
                }
            }).collect(Collectors.toMap(data -> (String) data[0], data -> (PolicyDocument)  data[1])));
        }
        ...
        if (serviceInputParameters.getDockerImageSource().isEcrSource()) {
            IRepository dockerRepository = Repository.fromRepositoryName(this, "ecrRepository", serviceInputParameters.getDockerImageSource().getDockerRepositoryName());
            dockerRepository.grantPull(ecsTaskExecutionRole);
            dockerRepositoryUrl = dockerRepository.repositoryUriForTag(serviceInputParameters.getDockerImageSource().getDockerImageTag());
        } else {
            dockerRepositoryUrl = serviceInputParameters.getDockerImageSource().getDockerImageUrl();
        }
        ...
        CfnTaskDefinition.ContainerDefinitionProperty container = CfnTaskDefinition.ContainerDefinitionProperty.builder()
                ...
                .cpu(serviceInputParameters.getCpu())
                .memory(serviceInputParameters.getMemory())
                .image(dockerRepositoryUrl)
                .logConfiguration(CfnTaskDefinition.LogConfigurationProperty.builder()
                        .logDriver("awslogs")
                        .options(Stream.of(new String[][] {
                                ...
                                {"awslogs-datetime-format", serviceInputParameters.getAwslogsDateTimeFormat()}
                        }).collect(Collectors.toMap(data -> data[0], data-> data[1])))
                        .build())
                .portMappings(singletonList(CfnTaskDefinition.PortMappingProperty.builder()
                        .containerPort(serviceInputParameters.getContainerPort())
                        .build()))
                .environment(toKeyValuePairs(serviceInputParameters.getEnvironmentVariables()))
                .build();

        CfnTaskDefinition taskDefinition = CfnTaskDefinition.Builder.create(this, "taskDefinition")
                // skipped family
                .cpu(String.valueOf(serviceInputParameters.getCpu()))
                .memory(String.valueOf(serviceInputParameters.getMemory()))
                .networkMode("awsvpc")
        ...
        allowIngressFromEcs(serviceInputParameters.getSecurityGroupIdsToGrantIngressFromEcs(), ecsSecurityGroup);
        ...
        CfnService service = CfnService.Builder.create(this, "ecsService")
                ...
                .deploymentConfiguration(CfnService.DeploymentConfigurationProperty.builder()
                        .maximumPercent(serviceInputParameters.getMaximumInstancesPercent())
                        .minimumHealthyPercent(serviceInputParameters.getMinimumHealthyInstancesPercent())
                        .build())
                .desiredCount(serviceInputParameters.getDesiredInstancesCount())
